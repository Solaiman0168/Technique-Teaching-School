...
<?php
    ...
    if(isset($_POST['update']))
    {
            ...
            $SQL = "SELECT * FROM $tbl_name WHERE  `bn_title` ='$bn_title' AND`en_title` ='$en_title' AND `home_heading4_id` != '$home_heading4_id'";
            $COUNT = mysqli_num_rows(mysqli_query($con,$SQL));
            if($COUNT < 1): 
                   
             $sql = "UPDATE $tbl_name SET `bn_title` = '$bn_title' ,`en_title` = '$en_title' ,`is_updated` = 1, `updated_by` = $user_no ,`updated_on` = '$CURR_TIME' WHERE home_heading4_id = 1";
