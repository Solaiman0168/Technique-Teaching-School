...
<?php $table_heading = "কৌতুক - ১";?>
<?php include 'include/sidebar.php';?>
<?php include 'include/body-top.php';?>
  
<?php
        $tbl_name="joke1";       //your table name
        $targetpage = "joke1.php";   //your file name  (the name of this file)
    ...
    {
        $ID = $_GET['delete'];
        $sql = "UPDATE $tbl_name SET `is_deleted` = 1 ,`deleted_by` = '$user_no', `deleted_on` = '$CURR_TIME' WHERE joke1_id = $ID";
        ...
        $SQL = "SELECT * FROM $tbl_name WHERE `is_deleted` = 0 AND `bn_name` ='$bn_name' AND `en_name` = '$en_name' AND `bn_student_name` = '$bn_student_name' AND `en_student_name` = '$en_student_name'AND `bn_class` = '$bn_class'AND `en_class` = '$en_class'AND `bn_date` = '$bn_date'AND `en_date` = '$en_date' AND `bn_details` = '$bn_details'AND `en_details` ='$en_details' AND `joke1_id` != '$joke1_id'";
            ...
            if($COUNT < 1): 
                if ($_FILES["image_url"]["error"] > 0) {
                  ...
                  $sql = "UPDATE $tbl_name SET `bn_name` ='$bn_name' , `en_name` = '$en_name' , `bn_student_name` = '$bn_student_name' , `en_student_name` = '$en_student_name', `bn_class` = '$bn_class', `en_class` = '$en_class',`bn_date` = '$bn_date', `en_date` = '$en_date' , `bn_details` = '$bn_details', `en_details` ='$en_details' , `image_url` = '$image_url' , `is_updated` = 1, `updated_by` = '$user_no' ,`updated_on` = '$CURR_TIME' WHERE `joke1_id` = '$joke1_id'";
                $result = mysqli_query($con,$sql);
                if($result)
                {
...
?>
    ...
    <?php
        if(isset($_GET['edit'])):
        $id = $_GET['edit'];
        $sql = "SELECT * FROM $tbl_name WHERE `joke1_id` = '$id' ";
    ...
    ?>
     ...
     <div class="form-group " <?php if($mgs=='')echo "style='display:none;'" ?>>
            ...
            <div>
                <input type="hidden" name="joke1_id" value="<?=$result['joke1_id']?>" />
            </div>
        </div>
...
?>
    ...
    <?php $i=$page*$limit-$limit+1; while($row = mysqli_fetch_array($result)):?>
        <tr>
            ...
            <td>
                <a onclick="return confirm('Are you Sure Want to Edit?');" href="<?=$targetpage.'?edit='.$row['joke1_id']?>" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                <a onclick="return confirm('Are you Sure Want to Delete?');" href="<?=$targetpage.'?delete='.$row['joke1_id']?>" class="btn btn-danger"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
            </td>
        </tr>
    <?php endwhile;?>
    </table>
