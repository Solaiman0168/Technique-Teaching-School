...
<?php $table_heading = "ছোট গল্প";?>
<?php include 'include/sidebar.php';?>
<?php include 'include/body-top.php';?>
  
<?php
        $tbl_name="short_story";       //your table name
        $targetpage = "short_story.php";   //your file name  (the name of this file)
    ...
    {
        $ID = $_GET['delete'];
        $sql = "UPDATE $tbl_name SET `is_deleted` = 1 ,`deleted_by` = '$user_no', `deleted_on` = '$CURR_TIME' WHERE short_story_id = $ID";
    ...
    if(isset($_POST['update']))
    {
            ...
            $bn_name = trim($_POST['bn_name']);
            $en_name = trim($_POST['en_name']);
            $short_story_id = $_POST['short_story_id'];
            $bn_student_name =trim( $_POST['bn_student_name']);
            $en_student_name = trim($_POST['en_student_name']);
            $bn_class =$_POST['bn_class'];
            $en_class =$_POST['en_class'];
        ...
        $SQL = "SELECT * FROM $tbl_name WHERE `is_deleted` = 0 AND `bn_name` ='$bn_name' AND `en_name` = '$en_name' AND `bn_student_name` = '$bn_student_name' AND `en_student_name` = '$en_student_name'AND `bn_class` = '$bn_class'AND `en_class` = '$en_class '  AND `bn_details` = '$bn_details'AND `en_details` ='$en_details' AND `short_story_id` != '$short_story_id'";
            ...
            if($COUNT < 1): 
                if ($_FILES["image_url"]["error"] > 0) {
                  ...
                  $sql = "UPDATE $tbl_name SET `bn_name` ='$bn_name' , `en_name` = '$en_name' , `bn_student_name` = '$bn_student_name' , `en_student_name` = '$en_student_name', `bn_class` = '$bn_class', `en_class` = '$en_class',`bn_date` = '$bn_date', `en_date` = '$en_date' , `bn_details` = '$bn_details', `en_details` ='$en_details' , `image_url` = '$image_url' , `is_updated` = 1, `updated_by` = '$user_no' ,`updated_on` = '$CURR_TIME' WHERE `short_story_id` = '$short_story_id'";
                $result = mysqli_query($con,$sql);
                if($result)
                {
...
?>
    ...
    <?php
        if(isset($_GET['edit'])):
        $id = $_GET['edit'];
        $sql = "SELECT * FROM $tbl_name WHERE `short_story_id` = '$id' ";
    ...
    ?>
     ...
     <div class="form-group " <?php if($mgs=='')echo "style='display:none;'" ?>>
            ...
            <div>
                <input type="hidden" name="short_story_id" value="<?=$result['short_story_id']?>" />
            </div>
        </div>
...
?>
    ...
    <?php $i=$page*$limit-$limit+1; while($row = mysqli_fetch_array($result)):?>
        <tr>
            ...
            <td>
                <a onclick="return confirm('Are you Sure Want to Edit?');" href="<?=$targetpage.'?edit='.$row['short_story_id']?>" class="btn btn-primary"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></a>
                <a onclick="return confirm('Are you Sure Want to Delete?');" href="<?=$targetpage.'?delete='.$row['short_story_id']?>" class="btn btn-danger"><i class="fa fa-trash-o" aria-hidden="true"></i></a>
            </td>
        </tr>
    <?php endwhile;?>
    </table>
